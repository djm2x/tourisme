...
import { Parcours, Quizz, Reponse } from 'src/app/Models/models';
...
export class DetailComponent implements OnInit {
  ...
  async doRespond(reponseUser: string, q: Quizz) {
    ...
    // console.log(new Date(r.date as any))
    if (r.id !== 0 && this.diff_minute(new Date(), r.date) < 1) {
      this.snackBar.notifyAlert(1, 'Vous devez au mois attendre une minute pour modifie ton reponse');
      this.disable = true;
      return;
    } else {
      this.disable = false;
    }
    console.log(r.id !== 0, this.diff_minute(new Date(), r.date))
