...
export class ParcoursComponent implements OnInit {
  ...
  createForm() {
    this.myForm = this.fb.group({
      ...
      userId: [this.session.user.id, Validators.required],
    });
  }
  ...
  async submit(o: Parcours) {
    ...
    if (this.id === 0) {
      this.uow.parcours.post(o).subscribe(async (r) => {
        ...
        this.o = r;
        // await this.uow.userParcoursCrees.post({userId: this.session.user.id, parcoursId: this.id, date: new Date()} as any).toPromise();
        this.isAdded = true;
        this.sendObjectToEtapComponent.next(this.o);
      });
    } else {
